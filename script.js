document.addEventListener("DOMContentLoaded", async () => {
  ...
  const populatePageContent = () => {
    ...
    const creator = settings.apiSettings?.creator || "Yudz Api'S"

    setPageContent(DOM.pageTitle, settings.name, "Yudz Api'S")
    setPageContent(DOM.wm, `© ${currentYear} Yudz Api'S, All rights reversed.`)
    setPageContent(DOM.appName, settings.name, "Yudz Api'S")
  ...
  DOM.modal.container.classList.remove("d-none"); // 👈 jangan hidden total
  ...
  if (paramContainer.children.length > 0) {
    ...
    DOM.modal.content.classList.remove("d-none"); // 👈 biar konten modal kebuka
    DOM.modal.spinner.classList.remove("d-none"); // bisa tunjukin loading
